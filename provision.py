...
# ── Azure provisioning ───────────────────────────────────────────────────────
def get_credential():
    """Interactive browser login via Entra ID (Azure AD)."""
    from azure.identity import InteractiveBrowserCredential
    info("Opening browser for Entra interactive login…")
    cred = InteractiveBrowserCredential()
    # Force a token fetch so the login dialog appears now
...
def provision_mysql(cfg: Config, cred, subnet):
    ...
    from azure.mgmt.rdbms.mysql_flexibleservers.models import (
        ...
        Storage, Backup, BackupGeoRedundantBackup,
    ...
    server_params = Server(
        ...
        backup=Backup(
            ...
            geo_redundant_backup=BackupGeoRedundantBackup.DISABLED,
        ),
        network=Network(
...
def main():
    ...
    if not args.skip_provision:
        if cfg.subscription_id == "YOUR_SUBSCRIPTION_ID":
            ...
            sys.exit(1)
        if cfg.admin_password == "Ch@ngeMe2024!":
